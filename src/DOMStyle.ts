import { UICell, UICellOffsets, UIColumn, UIComponent, UIContainer, UIControl, UIRow, UIScrollContainer, UISeparator, UIStyle, UITheme } from "typescene";
...
export function applyElementCSS(component: UIComponent, element: HTMLElement, isNewElement?: boolean, additionalClassName?: string) {
    ...
    if (component instanceof UIContainer) {
        ...
        if (component.shrinkwrap && component.dimensions.grow ||
...
export function getCSSLength(length?: UICellOffsets, defaultValue: any = "auto"): string {
    ...
    if (typeof length === "object") {
        let top: string | number = 0, bottom: string | number = 0;
        let left: string | number = 0, right: string | number = 0;
...
function defineStyleClass(style: UIStyle) {
    ...
    setGlobalCSS({
        [".UI" + style.ids.map(id => "." + id).join("")]:
            makeDeclaration(style.getOwnStyles())
    } as any);
    ...
    let setGlobalAddonStyle = (suffix: string, k: keyof UIStyle.ConditionalStyles) => {
        ...
        setGlobalCSS({
            [".UI" + style.ids.map(id => "." + id).join("") + suffix]:
                makeDeclaration(conditional.getStyles())
        } as any);
    }
...
function addContainerCSS(result: Partial<CSSStyleDeclaration>, container: UIContainer) {
    if (container instanceof UICell) {
        ...
        if (container.background) result.background = UITheme.replaceColor(container.background);
...
function addContainerLayoutCSS(result: Partial<CSSStyleDeclaration>, layout: UIStyle.ContainerLayout) {
    ...
    if (distribution !== undefined) result.justifyContent = _flexJustifyOptions[distribution] || "";
...
function addDimensionsCSS(result: Partial<CSSStyleDeclaration>, dimensions: UIStyle.Dimensions) {
...
function addTextStyleCSS(result: Partial<CSSStyleDeclaration>, textStyle: UIStyle.TextStyle) {
    ...
    if (lineBreakMode === "clip") result.overflow = "hidden", result.textOverflow = "clip";
    else if (lineBreakMode === "ellipsis") result.overflow = "hidden", result.textOverflow = "ellipsis";
    ...
    if (underline) result.textDecoration = "underline" + (strikeThrough ? " line-through" : "");
...
function addControlStyleCSS(result: Partial<CSSStyleDeclaration> & { className?: string },
...
function getCSSText(style: any) {
    ...
    for (let p in style) {
        ...
        let key = p.replace(/([A-Z])/g, "-$1").toLowerCase()
            .replace(/^(webkit|o|ms|moz)-/, "-$1-");
        for (var str of String(style[p]).split("||").reverse())
            result += key + ": " + str + "; ";
    }
    return result;
}
