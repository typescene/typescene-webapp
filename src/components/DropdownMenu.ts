import { UICell, UIComponent, UIComponentEvent, UIExpandedLabel, UILabel, UIMenuBuilder, UIMenuItemSelectedEvent, UIRenderableConstructor, UISeparator, UISpacer, UIStyle, UITransitionType } from "typescene";
...
export class DropdownMenuBuilder extends UIMenuBuilder {
    ...
    addOption(key: string, text: string, icon?: string, hint?: string, hintIcon?: string,
        ...
        this._items.push(
            UICell.with(
                ...
                hint ? UILabel.with({ text: hint, icon: hintIcon, textStyle: hintStyle }) : undefined,
    ...
    addSelectionGroup(options: Array<{ key: string, text: string }>,
        selectedKey?: string, textStyle?: Partial<UIStyle.TextStyle>) {
        for (let option of options) {
            this.addOption(option.key, option.text,
                option.key === selectedKey ? "check" : "blank",
                undefined, undefined, textStyle);
        }
        return this;
    }
