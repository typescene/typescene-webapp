import { observe, onPropertyChange, UICell, UIColumn, UIComponent, UIComponentEvent, UIContainer, UIFocusRequestEvent, UIRenderEvent, UIRow, UIScrollContainer, UIScrollEvent } from "typescene";
...
function hasComponentSpacing(container: UIContainer): container is UIContainerWithSpacing {
    return typeof (container as UIContainerWithSpacing).hasComponentSpacing === "function" &&
...
class UIContainerRenderer extends RendererBase<UIContainer, HTMLElement> {
    ...
    protected afterRender() {
        ...
        if (this.component instanceof UIScrollContainer) {
            ...
            if (this.isRendered()) {
                ...
                let lastEventT = 0, lastT = Date.now(), pending = false;
                ...
                let checkAndEmit = (e: Event) => {
                    ...
                    if (vertDist < 0) wentUp = true, wentDown = false;
                    if (vertDist > 0) wentUp = false, wentDown = true;
                    if (horzDist < 0) wentLeft = true, wentRight = false;
                    if (horzDist > 0) wentLeft = false, wentRight = true;
                    lastTop = element.scrollTop;
                    lastLeft = element.scrollLeft;
                    lastT = Date.now();
                    let event: UIScrollEvent;
                    if (lastEventT < lastT - 200) {
                        event = new UIScrollEvent("ScrollEnd",
                            this.component, undefined, e);
                        pending = false;
                    }
                    else {
                        event = new UIScrollEvent("Scroll",
                            this.component, undefined, e);
                    ...
                    event.atBottom = lastTop + element.clientHeight >=
                        element.scrollHeight - scrollContainer.bottomThreshold;
                    event.atRight = lastLeft + element.clientWidth >=
                        element.scrollWidth - scrollContainer.rightThreshold;
                    event.verticalVelocity = Math.abs(vertDist / (window.innerHeight || 1)) /
                        (tDiffSec || .1);
                    event.horizontalVelocity = Math.abs(horzDist / (window.innerWidth || 1)) /
                        (tDiffSec || .1);
    ...
    async onContentChangeAsync() {
        ...
        if (this.component.isKeyboardFocusable() &&
            this.lastFocused &&
    ...
    onAnimatedContentRenderingVelocityChange() {
        if (this._updater) {
            this._updater.setAnimationSpeed(
                this.component.animatedContentRenderingVelocity || 0);
        }
    }
    ...
    @onPropertyChange("hidden", "style", "layout", "dimensions", "padding",
        "height", "width", "verticalScrollEnabled", "horizontalScrollEnabled")
    ...
    onScrollEnd(e: UIComponentEvent) {
        ...
        if (element && (this.component instanceof UIScrollContainer)) {
            ...
            if (e.name === "ScrollEnd" ||
                ...
                let skew = e.name === "ScrollEnd" ? 0 :
                    e.name === "ScrollSnapUp" ? .25 : -.25;
            ...
            if (e.name === "ScrollEnd" ||
                e.name === "ScrollSnapLeft" || e.name === "ScrollSnapRight") {
                // snap horizontally
                let targetLeft: number | undefined;
                let furthest = rect.width;
                let skew = e.name === "ScrollEnd" ? 0 :
                    e.name === "ScrollSnapLeft" ? .25 : -.25;
    ...
    private _refreshUpdater(spacingOnly?: boolean) {
        ...
        if (element) {
            ...
            if (this.component.animatedContentRenderingDuration! >= 0) {
                this._updater.setAnimationTimeMs(
                    this.component.animatedContentRenderingDuration!);
            }
